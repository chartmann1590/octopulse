...
function isOctoPrintHeaders(headers: Headers): boolean {
  ...
  if (server.includes('octoprint') || server.includes('werkzeug') || server.includes('python')) return true;
...
async function probe(ip: string, port: number, timeoutMs = 2200): Promise<DiscoveryResult | null> {
  ...
  try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (j && (j.api || j.server || j.text || j.version)) {
          // Verify it's really OctoPrint JSON, not generic
          if (isOcto || j.api || j.server) {
            return { host: ip, port, name: j.server || j.text || `OctoPrint @ ${ip}`, version: j.api || j.text || j.version, via: 'scan' };
          }
        }
        // If 200 but not OctoPrint JSON, don't treat as found
      } else if (res.status === 401) {
        // Only treat 401 as OctoPrint if body/headers confirm it
        if (isOcto) {
          return { host: ip, port, name: `OctoPrint @ ${ip}:${port}`, version: 'needs API key', via: 'scan' };
        }
        // Also check if 401 body is JSON with OctoPrint error
        try {
    ...
    // 2) Try root / - must contain OctoPrint HTML
    try {
      ...
      if (res2.ok) {
        ...
        if (lower.includes('octoprint')) {
          ...
          if (lower.includes('<title') && lower.includes('octoprint')) {
            ...
            // Fallback: check for OctoPrint-specific divs
    ...
    // 3) Try /api/settings - also 401 only if OctoPrint
    try {
